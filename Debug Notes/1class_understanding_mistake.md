...
private:
    ...
    bool verify(int id){
        if(accountid==id){
            return true;    //return accountid==id;
        }
    }
public:
    ...
    void addmoney(vector<Bankaccount>& bankaccounts,int id,int add){
        for(int i=0;i<bankaccounts.size();i++){
            if(bankaccounts[i].verify(id)) {
                bankaccounts[i].property+=add;     //not regorous using  "for" here because 
            }
        }
    }
...
int main(){
    ...
    while(true){
        ...
        if(choice==1){
            ...
            cin>>id>>money;
            bankaccounts[0].addmoney(bankaccounts,id,money);// it only contains bankaccounts[0]'s own class atribute
        }
        else if(choice==3){
            int id;
            int money;
